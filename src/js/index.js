...
function initVersion() {
    document.querySelector('#version').innerHTML = pagejs.version
}
...
function initMarkdown() {
    document.querySelector('#doc').innerHTML = md_guide + md_configuration + md_parentRouter + md_childRouter
}
    ...
    function createTable(apiObject, headArray) {
        ...
        for (const apiName in apiObject) {
            if (apiObject.hasOwnProperty(apiName)) {
                ...
                el_td_type.innerHTML = apiItem.type instanceof Array ? apiItem.type.map(typeFun => `<code>${typeFun.name}</code>`).join(' |<br>') : apiItem.type ? `<code>${apiItem.type.name}</code>` : '任何类型'
                el_td_type.className = 'type'
                el_td_default.innerHTML = apiItem.default === extend ? '<code>继承globalRoute</code>' : `<code>${typeof apiItem.default === 'string' ? `'${apiItem.default}'` : JSON.stringify(apiItem.default)}</code>`
